...
'''
function to add Header to a given Page
takes PageID, text, font size as input
Builds the HTML content from text and font size
Header attributes - color:blue; text-decoration: underline
Returns PATCH Response(requests object) as output
'''
def updatePageHeader(myPageID, myText, myFontSize):
    ...
    patchPageBody=[
    {
        ...
        'action':'append',
        'position':'after',
        'content':'<br/><p style="font-size:'+myFontSize+'pt; color:blue; text-decoration: underline">'+myText+'</p>'
    }
    ]
...
'''
function to fetch pageID for today and add Header
takes as input myNotebook, myFontSize
If page doesn't exist, create page
function dependencies - getToken(), variousDates(), getPageID(), updatePageHeader()
'''
def addHeaderTodayPage(myNotebook, myFontSize):
    allDates=variousDates()
    pageName=str(allDates['dayName'])+', '+str(allDates['monthName'])+' '+str(allDates['day'])+', '+str(allDates['year'])
    ...
    if  myPageID == False:
        ...
        resp=updatePageHeader(newPageID, 'Enter New Heading', str(myFontSize))
    else:
        resp=updatePageHeader(myPageID, 'Enter New Heading', str(myFontSize))
        return resp

if __name__ == '__main__':
#MUST PROVIDE NOTEBOOK NAME AND FONT SIZE AS ARGUMENTS TO main()
    import sys
    myNotebook = sys.argv[1]
    myFontSize = sys.argv[2]
    resp=addHeaderTodayPage(myNotebook, myFontSize)