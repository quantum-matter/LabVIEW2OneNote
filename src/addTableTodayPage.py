...
Add a new header to Today's page
'''
...
'''
function to add table to a given Page from csv file data
takes PageID, csv file name as input
NOTE - CSV file should be TAB DELIMITED
Also takes a width file as input that custom sets the width of each column
Returns PATCH Response(requests object) as output
'''
import csv
def updatePageTable(myPageID, dataFile, widthFile):
    patchPageURL='https://graph.microsoft.com/v1.0/users/cqtasr@nus.edu.sg/onenote/pages/'+myPageID+'/content?includeIDs=true'
    patchPageHeaders={'Content-Type':'application/json', 'Authorization':'Bearer '+access_token}
    with open(dataFile, newline='', encoding = "ISO-8859-1") as csvfile1:
        myData = csv.reader(csvfile1, delimiter='\t')
        csvfile2 = open(widthFile, newline='', encoding = "ISO-8859-1")
        widthData = list(csv.reader(csvfile2, delimiter='\t'))

        HTMLTable='''<table style="border:0px;width:100%">
            '''
        for row in myData:
            count=0
            ...
            for column in row:
                try:
                	widthVal=widthData[0][count]
                except IndexError:
                	widthVal=50
                count+=1
                HTMLTable = HTMLTable + '''
                <td style="width:'''+str(widthVal)+'''">'''+str(column)+'''</td>
                '''
            HTMLTable = HTMLTable + '''</tr>
            '''
...
'''
function to fetch pageID for today and add table
takes as input myNotebook
function dependencies - getToken(), variousDates(), getPageID(), updatePageTable()
'''
def addTableTodayPage(myNotebook):
    allDates=variousDates()
    pageName=str(allDates['dayName'])+', '+str(allDates['monthName'])+' '+str(allDates['day'])+', '+str(allDates['year'])
    ...
    if  myPageID == False:
        ...
        resp=updatePageTable(newPageID, 'tempTable.csv', 'widthFile.csv')
    else:
        resp=updatePageTable(myPageID, 'tempTable.csv', 'widthFile.csv')
        return resp
...
#MUST PROVIDE NOTEBOOK NAME AS ARGUMENTS TO main()    
    ...
    resp=addTableTodayPage(myNotebook)