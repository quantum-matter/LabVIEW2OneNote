...
'''
function to add Text to a given Page
takes PageID, text, font size as input
Builds the HTML content from text and font size
Text attributes - color:blue; text-decoration: underline
Returns PATCH Response(requests object) as output
'''
def updatePageText(myPageID, myText, myFontSize):
    ...
    patchPageBody=[
    {
        ...
        'action':'append',
        'position':'after',
        'content':'<br/><p style="font-size:'+myFontSize+'pt; color:navy; text-decoration: underline"><b>'+myText+'</b></p>'
    }
    ]
...
'''
def addTextTodayPage(myNotebook, myFontSize, myText):
    allDates=variousDates()
    pageName=str(allDates['dayName'])+', '+str(allDates['monthName'])+' '+str(allDates['day'])+', '+str(allDates['year'])
    ...
    if  myPageID == False:
        ...
        resp=updatePageText(newPageID, myText, str(myFontSize))
    else:
        resp=updatePageText(myPageID, myText, str(myFontSize))
        return resp

if __name__ == '__main__':
#MUST PROVIDE NOTEBOOK NAME, FONT SIZE AND TEXT AS ARGUMENTS TO main()
    import sys
    ...
    resp=addTextTodayPage(myNotebook, myFontSize, myText)